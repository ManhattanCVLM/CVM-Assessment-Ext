...
- **A "What 1–8 mean" panel** on the card: all eight with their definitions, the chosen
  one marked. Opening it is remembered across the whole area, so it is opened once rather
  than per question, and it opens for a lapsed licence too — it is help, not an edit
...
2. **Bump `const CACHE` in `sw.js`** — currently `cvm-external-v24`
3. Commit and push
