# CVM External Assessment

The external assessment, on its own. 350 questions across 7 assessment areas, scored 1–8,
offline once installed, and nothing in it refers to the self assessment.

```
https://YOUR-USERNAME.github.io/CVM-Assessment-Ext/
```

Hand that URL to your external reviewers. It runs in any modern browser, on any device, and can be
...
**Answer the external assessment.** The default. There is no Self/External switch to leave
...
**Compare two external assessments.** *Setup → What this copy is doing → Comparing two
external assessments.* Import two finished ones — last year's against this year's — and
...
sending to someone. A file from an self assessment is refused rather than loaded under
the wrong name.
...
The word "self" does not appear in this app. Not hidden — absent:

- no assessment-type switch, and no self option anywhere
...
If a backup holding both assessments is restored here — the only way self scores can
...
Comparison is the exception that proves it: two series, but both are external assessments,
...
CVM External Assessment.html  one-file copy, for email or a laptop
...
2. **Bump `const CACHE` in `sw.js`** — currently `cvm-external-v12`
3. Commit and push
